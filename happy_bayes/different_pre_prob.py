...
def predict(summaries, inputVector):
    ...
    for classValue, probability in probabilities.items():
        # print("{}{}".format(classValue, probabilities))
        if bestLabel is None or probability > bestProb:
            bestProb = probability
            bestLabel = classValue
            # print('{}   {}'.format(bestProb, bestLabel))
...
def loading_test_data2(b, g):
    ...
    boy_data = pd.read_csv('./dates/boynew.txt', names=['height', 'weight', 'shoe size'], sep=' ')
    boy_data.insert(header.__len__(), 'class', [1] * boy_data.values.__len__())
    boy_data2 = pd.read_csv('./dates/boy.txt', names=['height', 'weight', 'shoe size'], sep='\t')
    boy_data2.insert(header.__len__(), 'class', [1] * boy_data2.values.__len__())
    boy_data = np.row_stack((np.array(boy_data.values), np.array(boy_data2.values)))[0:b]

    girl_data = pd.read_csv('./dates/girlnew.txt', names=['height', 'weight', 'shoe size'], sep=' ')
    girl_data.insert(header.__len__(), 'class', [0] * girl_data.values.__len__())
    girl_data2 = pd.read_csv('./dates/girl.txt', names=['height', 'weight', 'shoe size'], sep='\t')
    girl_data2.insert(header.__len__(), 'class', [0] * girl_data2.values.__len__())
    girl_data = np.row_stack((np.array(girl_data.values), np.array(girl_data2.values)))[0:g]

    print(boy_data.__len__(), girl_data.__len__())
    result_mat = np.vstack((boy_data, girl_data))
    print('[*]\t男女数据集比例（不同先验概率）{}:{}'.format(boy_data.__len__(), girl_data.__len__()))
    return result_mat
...
def ROCCurve_me(b, g):
    print("ROC曲线 by Me")
    boy_data = loading_test_data2(b, g)
    result = summarizeByClass(dataset=boy_data)
    boy_data = loading_test_data2(b, g)
    ...
    false_positive_rate, true_positive_rate, thresholds = roc_curve(result_list, sore * 1000)
    roc_auc = auc(false_positive_rate, true_positive_rate)
    return false_positive_rate, true_positive_rate, ccuracy, roc_auc


if __name__ == '__main__':
    plt.title('ROC by Me With different Pre')
    b_g_c = [
        (227, 108, 'b'),
        (392, 185, 'r'),
        (300, 100, 'y'),
    ...
    for i in b_g_c:
        ...
        b_rate = b / (b + g)
        g_rate = g / (b + g)
        false_positive_rate, true_positive_rate, ccuracy, roc_auc = ROCCurve_me(b, g)
        plt.plot(false_positive_rate, true_positive_rate, c,
                 label='AUC = %0.2f  Accuracy= %0.2f Pre = %s : %s' % (roc_auc, ccuracy, b, g))
