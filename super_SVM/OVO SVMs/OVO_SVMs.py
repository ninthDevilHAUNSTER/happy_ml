...
def ovo_svm_main(result_mat=np.array([]), class_label=np.array([]),
                 positive_label=1):
    b, alphas = smoP(result_mat, class_label, C=200, toler=0.001, maxIter=200, kTup=('lin', 1.3))
    ...
    for i in class_label_list:
        ...
        for j in range(two_part_class_label.__len__()):
            if two_part_class_label[j] == i:
                ...
                two_part_class_label[j] = -1
        b, ws = ovo_svm_main(X_train, two_part_class_label, positive_label=int(i))
        super_variables.append([b, ws])
        print("[*] one part done")
    pickle.dump(super_variables, open('./resultData', 'wb'))
